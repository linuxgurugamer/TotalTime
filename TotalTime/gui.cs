    ...
    public class MainMenuGui : MonoBehaviour
    {
...
#endif
        internal const string MODID = "TotalTime_NS";
        ...
        public void UpdateToolbarStock()
        {
            ...
            toolbarControl.AddToAllToolbars(GUIToggleToolbar, GUIToggleToolbar,
                ...
                MODID,
                "totalTimeButton",
                FileOperations.TEXTURE_DIR + "TotalTime-38",
                FileOperations.TEXTURE_DIR + "TotalTime-24",
               MODNAME
            );
        ...
        public void OnGUI()
        {
            if ((TotalTime.subscene || HighLogic.LoadedScene == GameScenes.EDITOR) && TotalTime.paused && TotalTime.lastScene != GameScenes.FLIGHT)
            {
                ...
                this.pauseBounds = ClickThruBlocker.GUILayoutWindow(GetInstanceID() + 2, pauseBounds, PauseWindow, "", b);
            }
            ...
            if (!TotalTime.F2)
            {
                if (this.configVisible)
                {
                    try
                    {
                        this.configBounds = ClickThruBlocker.GUILayoutWindow(GetInstanceID(), configBounds, ConfigWindow, TotalTime.TITLE, HighLogic.Skin.window);
                    ...
                    (TotalTime.config.displaySessionTime)))
                {
                    try
                    {
                        this.infoBounds = ClickThruBlocker.GUILayoutWindow(GetInstanceID() + 1, infoBounds, InfoWindow, TotalTime.TITLE, HighLogic.Skin.window);
                    }
                    catch (Exception)
                    {
                    }
